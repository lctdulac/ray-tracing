...
void save_image(const char* filename, const unsigned char* tableau, int w, int h) { // (0,0) is top-left corner // stored as RGB
...
int main()
{
	...
	for (int i = 0; i < H; i++){
		for(int j = 0; j < W; j++){
			img[(i * W + j)*3 + 0] = 255; // coordo rouge
			img[(i * W + j)*3 + 1] = 0; // coordo verte
			img[(i * W + j)*3 + 2] = 0; // coordo bleue
		}
	}
